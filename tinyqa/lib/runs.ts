...
export function buildInsights(runs: RunRecord[]): InsightsResponse {
  ...
  const passed_runs = runs.filter((run) => run.status === "passed").length;
  const failed_runs = runs.filter((run) => run.status === "failed").length;
  const timed_out_runs = runs.filter((run) => run.status === "timed_out").length;
  const manual_runs = runs.filter(
    (run) => run.source === "manual_live_preview"
  ).length;
  const webhook_runs = runs.filter((run) => run.source === "webhook").length;
  const pass_rate = total_runs > 0 ? Number(((passed_runs / total_runs) * 100).toFixed(2)) : 0;

  const failingRuns = runs.filter((run) =>
    ["failed", "error", "timed_out"].includes(run.status)
  );

  const failuresByRepoMap = new Map<string, number>();
  const failuresByStatusMap = new Map<RunStatus, number>();

  for (const run of failingRuns) {
    failuresByRepoMap.set(
      run.repo_full_name,
      (failuresByRepoMap.get(run.repo_full_name) ?? 0) + 1
    );
    failuresByStatusMap.set(run.status, (failuresByStatusMap.get(run.status) ?? 0) + 1);
  }
  ...
  return {
    ...
    webhook_runs,
    pass_rate,
    failures_by_repo: Array.from(failuresByRepoMap.entries())
      .map(([repo_full_name, count]) => ({ repo_full_name, count }))
      .sort((a, b) => b.count - a.count),
    failures_by_status: Array.from(failuresByStatusMap.entries())
